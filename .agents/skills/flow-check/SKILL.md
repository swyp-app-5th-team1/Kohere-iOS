...
Kohere 기능 구현 후 실제 Simulator에서 지정한 플로우를 조작해 검증한다. 필요하면 `ios-debugger-agent`를 사용하고, 사용자가 화면을 같이 봐야 하면 `ios-simulator-browser`를 병행한다.
...
3. `ios-debugger-agent`로 앱 빌드, 설치, 실행, 조작, 로그 확인을 진행한다.
4. 화면 확인이 중요하면 `ios-simulator-browser`를 함께 사용한다.
...
- `$ios-debugger-agent`: Simulator 빌드, 실행, 조작, 로그 확인
- `$ios-simulator-browser`: 사용자가 AI 조작 현황을 화면으로 따라봐야 할 때
